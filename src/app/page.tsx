      ...
      <section>
        <div className="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-12">
          <div className="max-w-5xl">
            ...
            <h1 className="mt-3 text-4xl font-semibold tracking-tight text-white sm:text-5xl">Main domains, categories, subcategories, companies.</h1>
          </div>

          <div className="mt-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <article className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--accent)]">Main domains</p>
              <p className="mt-2 text-2xl font-semibold text-white">{mainDomains.length}</p>
            </article>
            <article className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--accent)]">Subcategories</p>
              <p className="mt-2 text-2xl font-semibold text-white">{subcategoryCount}</p>
            </article>
            <article className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--accent)]">Companies</p>
              <p className="mt-2 text-2xl font-semibold text-white">{vendors.length}</p>
            </article>
            <article className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--accent)]">Verified companies</p>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {mainDomains.map((domain) => (
              <section key={domain.slug} className="rounded-2xl border border-[var(--border)] bg-[var(--card)] p-4">
                <div className="flex items-start justify-between gap-3">
                  <h2 className="text-base font-semibold tracking-tight text-white">{domain.name}</h2>
                  <span className="text-xs font-medium text-[var(--muted)]">{domain.categoryCount}</span>
                </div>
