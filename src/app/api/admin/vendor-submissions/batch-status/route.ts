...
import {
  ...
  updateVendorSubmissionStatuses,
...
const ADMIN_VENDOR_BATCH_STATUS_THROTTLE = {
  limit: 30,
  windowMs: 60000,
};

const ADMIN_VENDOR_BATCH_MAX_IDS = 200;
...
export async function POST(request: NextRequest) {
  try {
    const throttle = checkThrottle(
      "admin_vendor_batch_status",
      deriveThrottleKey(request.headers),
      ADMIN_VENDOR_BATCH_STATUS_THROTTLE,
    );
    if (!throttle.ok) {
    ...
    const source = payload.source || "submission_bulk";

    if (payload.ids.length > ADMIN_VENDOR_BATCH_MAX_IDS) {
      await appendQueueAuditEntries([
        createFailedQueueAuditEntry(
          "vendor_submission",
          "bulk",
          "Bulk vendor status update",
          "unknown",
    ...
    if (!payload.status || !isValidSubmissionStatus(payload.status)) {
      await appendQueueAuditEntries([
        createFailedQueueAuditEntry(
          "vendor_submission",
          "bulk",
      ...
      return NextResponse.json({ ok: false, updated: 0, missing: [], error: "Invalid submission status." }, { status: 400 });
    }

    const nextStatus = payload.status;

    const result = await updateVendorSubmissionStatuses(payload.ids, payload.status, {
      source,
    ...
    console.error("bulk submission status update failed", error);
    return NextResponse.json({ ok: false, updated: 0, missing: [], error: "Unable to update submission statuses." }, { status: 500 });
  }
}
...
function parseIdList(values: unknown, parser: (value: unknown) => string | null): string[] {
  ...
  if (Array.isArray(values)) {
    for (const value of values) {
      ...
      if (unique.size >= ADMIN_VENDOR_BATCH_MAX_IDS) {
        break;
      }
    }
...
export function GET() {
  return NextResponse.json({
    allowedStatuses: getSubmissionStatuses(),
  });
}
